...
MultiPlayerMenu(lastIP=false, lastPort=false, ByRef networkType=false, ByRef networkPlayers=0, setupNetwork=false, keyboardControl=true, severUseNetworkIP=true, disableSeverInfoMenu=false, textMessage=false,localLink=false, serverMode="portOnly") {
	...
	If (currentNumberOfPlayers){
		...
		Log("MultiPlayer - User selected the number of players equal to: " . networkPlayers)
		if (networkPlayers>1)
			networkSession := true
		else
    ...
    CloseMultiplayerTextMessage:
        Log("MultiPlayer - User canceled the text message. Exiting RocketLauncher session.")
        ExitModule()
	Return	
	...
	CloseMultiplayerMenu:
		Log("MultiPlayer - User pressed exit key. Exiting RocketLauncher session.")
	...
	CloseIPMenu:
		...
		If (previousMultiplayerMenu[multiplayerMenulevel]="MultiPlayerSelectionMenu"){
			Log("MultiPlayer - User pressed the exit key to cancel the IP Selection Menu. Back to the Multi Player Selection Menu.")
			multiplayerMenulevel--
			Gosub, DrawMultiPlayerSelectionMenu
		} Else If (previousMultiplayerMenu[multiplayerMenulevel]="PlayerSelectionMenu"){
			Log("MultiPlayer - User pressed the exit key to cancel the IP Selection Menu. Back to the Number of Players Selection Menu.")
			multiplayerMenulevel--
	...
	ClosePlayerNumberMenu:
		...
		If (previousMultiplayerMenu[multiplayerMenulevel]="MultiPlayerSelectionMenu"){
			Log("MultiPlayer - User pressed the exit key to cancel the Number of Players Selection Menu. Back to the Multi Player Selection Menu.")
			multiplayerMenulevel--
			Gosub, DrawMultiPlayerSelectionMenu
		} Else {
			Log("MultiPlayer - User pressed exit key. Exiting RocketLauncher session.")
	...
	CloseServerPortMenu:
		...
		If (previousMultiplayerMenu[multiplayerMenulevel]="MultiPlayerSelectionMenu"){
			Log("MultiPlayer - User pressed the exit key to cancel the Sever Port Menu. Back to the Multi Player Selection Menu.")
			multiplayerMenulevel--
			Gosub, DrawMultiPlayerSelectionMenu
		} Else If (previousMultiplayerMenu[multiplayerMenulevel]="PlayerSelectionMenu"){
			Log("MultiPlayer - User pressed the exit key to cancel the Sever Port Menu. Back to the Number of Players Selection Menu.")
			multiplayerMenulevel--
			Gosub, DrawPlayerSelectionMenu
		} Else {
			Log("MultiPlayer - User pressed exit key. Exiting RocketLauncher session.")
	...
	CloseServerInfoMenu:
		...
		If (previousMultiplayerMenu[multiplayerMenulevel]="ServerPortMenu"){
			Log("MultiPlayer - User pressed the exit key to cancel the Sever Info Menu. Back to the Server Port Menu.")
			multiplayerMenulevel--
			Gosub, DrawServerPortMenu
		} Else {
			Log("MultiPlayer - User pressed exit key. Exiting RocketLauncher session.")
	...
	CloseSetupNetworkMenu:
		...
		If (previousMultiplayerMenu[multiplayerMenulevel]="IPSelectionMenu"){
			Log("MultiPlayer - User pressed the exit key to cancel the Setup Network Menu. Back to the IP Selection Menu.")
			multiplayerMenulevel--
			Gosub, DrawIPSelectionMenu
		} Else If (previousMultiplayerMenu[multiplayerMenulevel]="ServerInfoMenu"){
			Log("MultiPlayer - User pressed the exit key to cancel the Setup Network Menu. Back to the Server Info Menu.")
			multiplayerMenulevel--
			Gosub, DrawServerInfoMenu
		} Else {
			Log("MultiPlayer - User pressed exit key. Exiting RocketLauncher session.")
