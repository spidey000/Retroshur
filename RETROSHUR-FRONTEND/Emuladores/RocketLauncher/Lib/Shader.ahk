...
RepeatBitmap(pBitmap, width, height, order=4, timeout=5000) {
	if(!(width) or !(height)){
		Log("Shader - width or height information missing on the code. Shader will not be displayed.",2)
		Return
	}
	...
	{	Gdip_GetImageDimensions(pBitmap, origW, origH)
		...
		if(startTime < A_TickCount - timeout) {
			Log("Shader - Shader will not be displayed because timeout was reached on the shader creation process.",2)
			Break
		}
	}
	Return pBitmap
