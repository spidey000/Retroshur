...
CloseFadeIn:
	...
	If ErrorLevel {
		...
		Log("User cancelled 7z extraction. Ending RocketLauncher and returning to Front End",3)
...
AnykeyFadeBypass:
	If (A_TimeIdlePhysical <= anykeyStart) {	; If our current idle time is less then the amount we started, user must of pressed a key and we should exit fade
		Log("AnykeyFadeBypass - User interrupted Fade_" . anykeyMethod . ". No longer idle. Skipping to " . (If anykeyMethod = "in" ? "FadeInExit" : "FadeOutExit"))
		fadeInterrupted = 1
	...
	{	currentJoy := A_index
		...
		If (mbtn = "D") {
			Log("AnykeyFadeBypass - User interrupted Fade_" . anykeyMethod . ". Mouse button pressed. Skipping to " . (If anykeyMethod = "in" ? "FadeInExit" : "FadeOutExit"))
			fadeInterrupted = 1
			SetTimer, AnykeyFadeBypass, Off	; shut off this timer
			Goto, % (If anykeyMethod = "in" ? "CloseFadeIn" : "CloseFadeOut")
		}
	...
	{	Loop, %JoystickNumber%
		{	currentJoy := A_index
			...
			{	GetKeyState, joy%a_index%, %currentJoy%joy%a_index%
				If joy%a_index% = D
				{	Log("AnykeyFadeBypass - User interrupted Fade_" . anykeyMethod . ". Joystick interrupted fade. Skipping to " . (If anykeyMethod = "in" ? "FadeInExit" : "FadeOutExit"))
					buttons_down = true
					fadeInterrupted = 1
					SetTimer, AnykeyFadeBypass, Off	; shut off this timer
					Goto, % (If anykeyMethod = "in" ? "CloseFadeIn" : "CloseFadeOut")
...
CustomKeyFadeBypass:
	Log("CustomKeyFadeBypass - User interrupted Fade_" . customKeyMethod . ", skipping to " . (If customKeyMethod = "in" ? "FadeInExit" : "FadeOutExit"))
	fadeInterrupted = 1
...
FadeInStart:
	...
	{	Log("FadeInStart - Started",4)
		...
		If (fadeInterruptKey = "anykey") {	; if user wants anykey to be able to disrupt fade, use this label
			Log("FadeInStart - Any key will interrupt this fade process",4)
			...
			Log("FadeInStart - Only these keys will interrupt this fade process: " . fadeInterruptKey,4)
			customKeyMethod = in
			XHotKeywrapper(fadeInterruptKey,"CustomKeyFadeBypass","ON")
		} Else {
			Log("FadeInStart - No keys were set to interrupt fade so ability to interrupt fade is disabled",4)
		}
		...
		Log("FadeInStart - Fade screen scale factor: X=" . fadeXScale . ", Y= " . fadeYScale,5)
...
FadeOutStart:
	...
	{	Log("FadeOutStart - Started",4)
		...
		If (fadeInterruptKey = "anykey") {	; if user wants anykey to be able to disrupt fade, use this label
			Log("FadeOutStart - Any key will interrupt this fade process",4)
			anykeyStart := A_TimeIdlePhysical	; store current idle time so AnykeyFadeBypass timer knows if it has been reset
			...
			Log("FadeOutStart - Only these keys will interrupt this fade process: " . fadeInterruptKey,4)
			customKeyMethod = out
			XHotKeywrapper(fadeInterruptKey,"CustomKeyFadeBypass","ON")
		} Else {
			Log("FadeOutStart - No keys were set to interrupt fade so ability to interrupt fade is disabled",4)
		}
		...
		If (fadePicPath%a_index%)
			...
			If (fadePicList[1]) ; if we filled anything in the array, stop here, randomize pics found	, and return
				{ 	Random, RndmfadePic, 1, % fadePicList.MaxIndex()
					...
					Log("GetFadePicFile - Randomized images and Fade " . name . " will use " . fadePicFile)
					break
			}
		}
	}
...
GetFadeDirPicFile(name,path,supportedFileTypes="png|gif|tif|bmp|jpg"){
	Log("GetFadeDirPicFile - Checking if any Fade """ . name . """ media exists in: " . path . "*.*",4)
	fadePicList := []
	If (FileExist(path . "*.*")) {
		Loop, Parse, supportedFileTypes,|
		{	Log("GetFadeDirPicFile - Looking for Fade """ . name . """: " . path . "*." . A_LoopField,4)
			Loop, % path . "*." . A_LoopField
			{	Log("GetFadeDirPicFile - Found Fade """ . name . """: " . A_LoopFileFullPath,4)
