	...
	If (bezelEnabled = "true"){
		...
		If bezelLayoutFile	
			...
			If !FileExist( emuPath . "\artwork\" . bezelLayoutFile . ".zip") and !FileExist( emuPath . "\artwork\" . parent . ".zip")
				{
				Log("Bezel - Layout mode selected but no MAME or MESS layout file found. Using RocketLauncher Bezel normal mode instead.",1)
				bezelMode = Normal
				bezelLayoutFile =
			} Else {
        ...
        If (bezelMonitor > monitorTable.MaxIndex())
            bezelMonitor := primMonitor
		Log("Bezel - Game will be moved to monitor " . bezelMonitor . " if RL finds a valid bezel to be show.",1)
		...
		If bezelPath 
			...
			If (extraFullScreenBezel = "true"){
				Log("Bezel - Adding extra fullscreen fake bezel image",1)
				Log("Bezel - This could prevent real bezels froms showing! Disable extraFullScreenBezel setting to restore real bezels if you don't want to use the bezel fullscreen mode.",2)
		...
		If bezelPath 
			...
			if (shaderObj){		
				If (bezelMode = "MultiScreens"){
					...
					Gdip_DrawImage(Bezel_G3, shaderBitmap, 0, 0, bezelScreenWidth, bezelScreenHeight)
				}
				Log("Bezel - Loading Shader: " . RLMediaPath . "\Shaders\" . shaderName,1)
			}
			...
			If (bezelBackgroundFile) {
				...
				Log("Bezel - Background Screen Position: BezelImage left=" . 0 . " top=" . 0 . " right=" . monitorTable[bezelMonitor].Width . " bottom=" . monitorTable[bezelMonitor].Height ,5)
			}
			If !(bezelLoaded) {				
			...
			;------------ bezelMode Normal
				...
				If !disableWinMove
					...
					Loop
						...
						{
							Log("Bezel - Moving emulator window delay, window moved",5)
							Break
						}
						if(timeout < A_TickCount - bezelCheckPosTimeout)
						{
							Log("Bezel - Moving emulator window delay timed out",5)
							Break
						}
						Sleep, 50
						WinMove, ahk_id %emulatorID%, , %bezelScreenX%, %bezelScreenY%, %bezelScreenWidth%, %bezelScreenHeight%
			...
			;------------ bezelMode fixResMode
				...
				If !disableWinMove
					...
					timeout := A_TickCount
					Sleep, 200
					Log("Bezel - Moving emulator window delay start",5)
					Loop
						{
						Sleep, 50
			...
			if (shaderObj){
				If !(bezelLoaded) {
					If (bezelMode = "MultiScreens")
						...
						UpdateLayeredWindow(Bezel_hwnd3, Bezel_hdc3,bezelScreenX,bezelScreenY, bezelScreenWidth, bezelScreenHeight)
				}
			}
...
BezelExit(){
	...
	If (bezelEnabled = "true"){
		...
		If bezelPath 
			{
			log("Bezel - Removing bezel image components to exit RocketLauncher.",1)
			If bezelBitmap
...
If bezelPath
	...
	Loop {
		...
		if (shaderObj){
			If (bezelMode = "MultiScreens")
				...
				UpdateLayeredWindow(Bezel_hwnd3, Bezel_hdc3,bezelScreenX,bezelScreenY, bezelScreenWidth, bezelScreenHeight,t)
		}
		If bezelOverlayFile
			{
			If (bezelMode = "MultiScreens")
	...
	bezelImageFile := bezelImagesList[RndmBezel]	
	If (bezelImageFile = "fakeFullScreenBezel"){
		Log("Bezel - Loading fake full screen Bezel.",1)
		origbezelImageW := monitorTable[bezelMonitor].Width
		origbezelImageH := monitorTable[bezelMonitor].Height
		...
		Log("Bezel - Loading Bezel: " . bezelImageFile,1)
		bezelBitmap := Gdip_CreateBitmapFromFile(bezelImageFile)
		Gdip_GetImageDimensions(bezelBitmap, origbezelImageW, origbezelImageH)
		ReadBezelIniFile()
		;loading background
