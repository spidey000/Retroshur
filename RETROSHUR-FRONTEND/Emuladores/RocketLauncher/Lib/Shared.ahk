...
StartModule(){
	...
	RLLog.Info(A_ThisFunc . " - MEmu: " . MEmu . "`r`n`t`t`t`t`tMEmuV: " . MEmuV . "`r`n`t`t`t`t`tMURL: " . MURL . "`r`n`t`t`t`t`tMAuthor: " . MAuthor . "`r`n`t`t`t`t`tMVersion: " . MVersion . "`r`n`t`t`t`t`tMCRC: " . MCRC . "`r`n`t`t`t`t`tiCRC: " . iCRC . "`r`n`t`t`t`t`tMID: " . MID . "`r`n`t`t`t`t`tMSystem: " . MSystem)
...
ExitModule(){
	Global statisticsEnabled,keymapperEnabled,keymapper,keymapperAHKMethod,keymapperLoaded,logShowCommandWindow,pToken,cmdWindowObj,mouseCursorHidden,cursor,hideCursor,rocketLauncherIsExiting,servoStikExitMode,ledblinkyEnabled,RLLog,zz
...
WinWait(winTitle,winText="",secondsToWait=30,excludeTitle="",excludeText=""){
	Global detectFadeErrorEnabled,logLevel,RLLog
	If logLevel > 3
...
WinWaitActive(winTitle,winText="",secondsToWait=30,excludeTitle="",excludeText=""){
	Global detectFadeErrorEnabled,logLevel,emulatorProcessID,emulatorVolumeObject,emulatorInitialMuteState,fadeMuteEmulator,fadeIn,RLLog
	If (logLevel > 3)
		GetActiveWindowStatus()
	RLLog.Info(A_ThisFunc . " - Waiting for """ . winTitle . """")
...
RunWait(target,workingDir="",options=0,ByRef outputVarPID=""){
	Global errorLevelReporting,RLLog
...
Run(target,workingDir="",options=0,ByRef outputVarPID="",inputBlocker=1,bypassCmdWindow=0,disableLogging=0){
	...
	Global logShowCommandWindow,logCommandWindow,cmdWindowObj,blockInputTime,blockInputFile,errorLevelReporting,RLLog
	options := If options = 1 ? "useErrorLevel" : options	; enable or disable error level
	If disableLogging
...
CheckFile(file,msg="",timeout=6,crc="",crctype="",logerror="",allowFolder=0){
	Global RLObject,logIncludeFileProperties,RLLog
	exeFileInfo := "
	( LTrim
	FileDescription
	FileVersion
...
ScriptError(error,timeout=6,w=800,h=225,txt=20,noexit=""){
	Global RLMediaPath,exitEmulatorKey,RLFile,RLErrSoundPath,logShowCommandWindow,cmdWindowObj,scriptErrorTriggered,RLLog,errorSounds
...
Log(txt,lvl=1,notime="",dump="",firstLog=""){
	...
	Global logFile,logLevel,logLabel,logShowDebugConsole,RLLog
	RLLog[x[lvl]](txt)
}
...
RomNameExistCheck(file,archivesOnly="") {
	Global romPathFromIni,romExtensions,sevenZFormats,RLLog
...
CreateRomTable(table) {
	Global romPathFromIni,dbName,romExtensionOrig,sevenZEnabled,romTableStarted,romTableComplete,romTableCanceled,rocketLauncherIsExiting,mgCandidate,RLLog
	romTableStarted := 1
	romTableCanceled := ""
	romTableComplete := ""
...
RomTableCheck() {
	Global systemName,mgEnabled,pauseEnabled,romTable,romTableStarted,romTableComplete,romTableCanceled,mgCandidate,dbName,RLLog
...
LEDBlinky(mode) {
	Global ledblinkyEnabled,ledblinkySystemName,ledblinkyFullPath,ledblinkyProfilePath,ledblinkyRLProfile,dbName,systemName,romName,RLLog
...
KeymapperProfileSelect(mode,tool,path,ext,type="") {
	Global keyboardEncoderEnabled,ultraMapEnabled,profilePath,keymapperFrontEndProfileName,systemName,emuName,dbName,RLLog
...
ServoStik(direction) {
	Global PacDriveDllFile,servoStikEnabled,RLLog
	RLLog.Info(A_ThisFunc . " - Started")
	Static dllExists
...
DxwndUpdateIniPath() {
	Global dxwndFullPath,dxwndPath,dxwndIni,systemName,RLLog
...
{	Global mouseCursorHidden,RLLog
...
SetSystemCursor( Cursor = "", cx = 0, cy = 0 ) {
	...
	{
		RLLog.Debug("SetSystemCursor - Creating blank cursor")
		VarSetCapacity( AndMask, 32*4, 0xFF ), VarSetCapacity( XorMask, 32*4, 0 )
		BlankCursor := 1 ; flag for later
	}
	Else If SubStr( Cursor,1,4 ) = "IDC_" ; load system cursor
	{
		RLLog.Debug("SetSystemCursor - Loading system cursor")
		...
		{
			RLLog.Warning("SetSystemCursor - Invalid cursor name supplied: """ . CursorHandle . """")
	...
	{
		RLLog.Debug("SetSystemCursor - Found this cursor: """ . Cursor . """")
		...
		{
			RLLog.Warning("SetSystemCursor - Invalid cursor extension: """ . Ext . """")
	...
	Else
	{   
		RLLog.Warning("SetSystemCursor - Invalid cursor name or path: """ . Cursor . """")
...
NumberOfPlayersSelectionMenu(maxPlayers=4) {
	Global screenRotationAngle,baseScreenWidth,baseScreenHeight,xTranslation,yTranslation,RLLog
...
HideEmuStart(ms=2) {
	Global hideEmu,hideEmuObj,RLLog
	If (hideEmu = "true")
...
{	Global Pause_Active,Pause_Running,systemName,gameInfo,RLLog
...
BroadcastMessage(StringToSend) {
	Global broadcastWindowTitle,RLLog
