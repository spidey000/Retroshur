...
Pause_Main:
    ...
    RLLog.Debug("Disabled exit emulator, bezel, and multigame keys")
    ...
    If (Pause_Loaded <> 1){ ; Initiate Gdip+ If first Pause run
        If !pToken := Gdip_Startup()
            RLLog.Error("gdiplus error!, Gdiplus failed to start. Please ensure you have gdiplus on your system")
        RLLog.Debug("Started Gdip " pToken " (If number -> loaded)")
    }
	StartPauseUserFeatures%zz%()	; starting pause user functions here so they are triggered before Pause starts
    ...
    If (RIni_Read("P",Pause_GlobalFile) = -11) {
        RLLog.Debug("Global Pause.ini file not found, creating a new one.")
        RIni_Create("P")
    }
    If (RIni_Read("P_sys",Pause_SystemFile) = -11) {
        ...
        RLLog.Debug( A_ScriptDir . "\Settings\" . systemName . "\Pause.ini file not found, creating a new one.")
        RIni_Create("P_sys")
	}
    ...
    RLLog.Debug("Loaded Emulator information: EmulatorProcessName: " emulatorProcessName ", EmulatorClass: " EmulatorClass ", EmulatorID: " EmulatorID)
    ...
    If((Pause_MuteWhenLoading="true") or (Pause_MuteSound="true")){ 
        ...
        getMute(PauseInitialMuteState,emulatorVolumeObject)
        If !(PauseInitialMuteState){
            setMute(1,emulatorVolumeObject)
            RLLog.Debug("Muting emulator sound while Pause is loaded. Master Mute status: " getMute(,emulatorVolumeObject) " (1 is mutted)")
        }
    }
    ...
    if !(Pause_Loaded){
        Pause_MainMenu_Labels := RIniPauseLoadVar("P","P_sys", "Main Menu Appearance Options", "Main_Menu_Items", "Controller|Change Disc|Save State|Load State|HighScore|Artwork|Guides|Manuals|Videos|Sound|Statistics|Moves List|History|Settings|Shutdown")
    ...
    RLLog.Info("Pause Started: current rom: " dbName ", current system Name: " systemName)
    RLLog.Debug("Created Black Screen to hide FrontEnd")
    Gosub, HaltEmu ;getting system specific commands from modules and pausing the emulator 
    RLLog.Debug("Loaded emulator specific module start commands")
    ...
    if !(Pause_Loaded){
        ...
        RLLog.Debug("Pause screen scale factor: X=" . Pause_XScale . ", Y= " . Pause_YScale)
    }
    ...
    RLLog.Debug("Setting Pause starting time for subtracting from statistics played time: " Pause_BeginTime)
    ...
    RLLog.Debug("Loading screen created")
    ...
    If !(Pause_Loaded){
        ...
        RLLog.Debug("Loaded Pause options")
        PauseOptionsScale() ;Setting scalling parameters and scalling variables        
        RLLog.Debug("Scaled Pause variables")
        gosub, FirstTimePauseRun ;Loading variables on first run        
        RLLog.Debug("Initilized Pause variables for the first time")
    ...
    If (pauseOnPrimaryMonitor or !Pause_Loaded) {   ; initialize pause menu if pause is not already drawn to secondary screen
        ...
        RLLog.Debug("Initialized Pause brushes and guis")
        ...
        RLLog.Debug("Loaded Main Menu Background and infos")
        ...
        RLLog.Debug("Loaded Main Menu Bar. Current Main Menu Label: " Pause_MainMenuSelectedLabel)
        If(Pause_MainMenu_ShowClock="true"){ ;Drawing the clock
            SetTimer, Clock, 1000
            RLLog.Debug("Loaded Clock")
        }
    }
        ...
        If(Pause_MuteWhenLoading="true"){ ;Unmuting If initial state was unmuted
            If !(PauseInitialMuteState){
                ...
                If(CurrentMuteState=1){
                    ...
                    RLLog.Debug("Unmuting computer sound while Pause is loaded. Master Mute status: " getMute(,emulatorVolumeObject) " (0 is unmutted)")
                }
            }  
        }
    }   
	If(Pause_MusicPlayerEnabled = "true"){ ;Loading music player 
        ...
        RLLog.Debug("Loaded Music Player")
    }
    ...
    RLLog.Info("Finished Loading Pause")
...
FirstTimePauseRun: ;Loading pause menu variables (first time run only)
    ...
    ;checking for bad written labels and non included labels (and adding them to the end of Pause_MainMenu_Labels)
    CheckedPause_MainMenu_Labels := ""
    Loop, parse, Pause_MainMenu_Labels,|
        {
    ...
    If !(CheckedPause_MainMenu_Labels = Pause_MainMenu_Labels . "|")
        RLLog.Warning("You have a Main Menu item not found or bad written in the Pause_MainMenu_Labels items list:`r`n`t`t`t`t`t Original Ini Main Menu list: " Pause_MainMenu_Labels "`r`n`t`t`t`t`t Corrected Main Menu list:    " CheckedPause_MainMenu_Labels)
    Pause_MainMenu_Labels := CheckedPause_MainMenu_Labels
    ; removing menu items not needed on Pause only call 
    If (rlMode = "pause"){
        if InStr(Pause_MainMenu_Labels,"Save State")
            StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, Save State|,
        if InStr(Pause_MainMenu_Labels,"Load State")
            StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, Load State|,
        if InStr(Pause_MainMenu_Labels,"Change Disc")
            StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, Change Disc|, ;
    }
    ...
    RLLog.Debug("Starting Creating Pause Contents Object.")
    ...
    Loop, parse, Pause_MainMenu_Labels,|, ;Loading Submenu information and excluding empty sub menus
        {
        StringReplace, temp_mainmenulabel, A_LoopField, %A_SPACE%,, All
        If (temp_mainmenulabel="Artwork"){
            RLLog.Debug("Loading Artwork Contents")
            ...
            If (Pause_ArtworkMenuEnabled="true"){
                ...
                StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, Artwork|, ;Removing Artwork menu If user defined to not show it
            }
        }
        If (temp_mainmenulabel="Controller"){
            RLLog.Debug("Loading Controller Contents")
            If (Pause_ControllerMenuEnabled="true"){
                ...
                StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, Controller|, ;Removing Controller menu If user defined to not show it
            }
        }
        If ((temp_mainmenulabel="SaveState")or(temp_mainmenulabel="LoadState")){
            RLLog.Debug("Loading " temp_mainmenulabel " Contents")
            If (Pause_SaveandLoadMenuEnabled="true"){
                ...
                if (PauseMediaObj[temp_mainmenulabel].TotalLabels<1){
        ...
        If (temp_mainmenulabel="ChangeDisc"){
            RLLog.Debug("Loading Change Disc Contents")
            If (Pause_ChangeDiscMenuEnabled="true"){
                ...
                If (Totaldiscsofcurrentgame>1){
                    ...
                    StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, Change Disc|, ;Removing change disc submenu If the game is not a multi disc game  
                }
            } Else {
                StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, Change Disc|, ;Removing change disc submenu If user defined to not show it
            }
        }
        If(temp_mainmenulabel="HighScore"){
            RLLog.Debug("Loading HighScore Contents")
            If(Pause_HighScoreMenuEnabled="true"){
                ...
                IfNotInString, HighScoreText, RANK ;Removing high score submenu If no high score information is found
                    {
                    StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, HighScore|,
                }                
        ...
        If(temp_mainmenulabel="MovesList"){
            RLLog.Debug("Loading MovesList Contents")
            If(Pause_MovesListMenuEnabled="true"){
                If FileExist(Pause_MovesListDataPath . "\" . systemName . ".dat") ;Loading Moves List
                    ...
                    If RomCommandDatText
                        ...
                        StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, Moves List|, ;Removing the moves list submenu If the game is not founded in the system.dat 
                    }
                } Else {
                    StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, Moves List|, ;Removing the moves list submenu If the system.dat is not found
                }
            } Else {
                StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, Moves List|, ;Removing the moves list submenu If user defined to not show it
            }
        }
        If(temp_mainmenulabel="Guides"){
            RLLog.Debug("Loading Guides Contents")
            If(Pause_GuidesMenuEnabled="true"){
                ...
                StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, Guides|, ;Removing the guides submenu If user defined to not show it
            }
        }
        If(temp_mainmenulabel="Manuals"){
            RLLog.Debug("Loading Manuals Contents")
            If(Pause_ManualsMenuEnabled="true"){
                ...
                StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, Manuals|, ;Removing the manuals submenu If user defined to not show it
            }
        }
        If(temp_mainmenulabel="Videos"){
            RLLog.Debug("Loading Videos Contents")
            If(Pause_VideosMenuEnabled="true"){
                ...
                StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, Videos|, ;Removing the videos submenu If user defined to not show it
            }
        }        
        If (temp_mainmenulabel="Sound"){
            If (Pause_SoundMenuEnabled="true")
                RLLog.Debug("Loading Sound Contents")
            Else
                StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, Sound|, ;Removing the sound submenu If user defined to not show it
        }     
        If (temp_mainmenulabel="Settings"){
            If (Pause_SettingsMenuEnabled="true")
                RLLog.Debug("Loading Settings Menu Contents")
            Else
                StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, Settings|, ;Removing the sound submenu If user defined to not show it
        }     
        If(temp_mainmenulabel="Statistics"){
            If  (statisticsEnabled = "true")
                {
                If (Pause_StatisticsMenuEnabled="true"){
                    RLLog.Debug("Loading Statistics Contents")
                    LoadingText("Loading Statistics...")
                    if !statisticsLoaded 
                        gosub, LoadStatistics ;Load Game Statistics Information
                    CreatingStatisticsVariablestoSubmenu()
                } Else {
                    StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, Statistics|, ;Removing the Statistics submenu If user defined to not show it
                }
            } Else { 
               StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, Statistics|, 
           }
        }    
        If (temp_mainmenulabel="History"){
            RLLog.Debug("Loading History.dat Contents")
            If (Pause_HistoryMenuEnabled="true"){
                ...
                StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, History|, ;Removing the History Dat submenu If user defined to not show it
            }
        }
        If(temp_mainmenulabel="Shutdown"){
            If(Pause_ShutdownLabelEnabled="true"){
                RLLog.Debug("Adding Shutdown Label")
            } Else {
                StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, Shutdown|, ;Removing Artwork menu If user defined to not show it
            }
        }        
    }    
    ...
    ;Array_List(feMedia)
    if InStr(Pause_MainMenu_Labels,"Videos")
        loop, % PauseMediaObj["Videos"].TotalLabels
            VideoPosition%a_index% := 0
    StringTrimRight, Pause_MainMenu_Labels, Pause_MainMenu_Labels, 1 ;Counting total Main Menu items
    Loop, parse, Pause_MainMenu_Labels,|, 
        {
        TotalMainMenuItems := A_Index
    }
    RLLog.Info("Pause Menu items: " Pause_MainMenu_Labels)
    ...
    Loop, parse, Pause_MainMenu_Labels,|, ;initializing auxiliar page tracking
        {
        StringReplace, temp_mainmenulabel, A_LoopField, %A_SPACE%,, All
    ...
    Loop, parse, Pause_MainMenu_Labels,|, 
        {
        Widthoftext := MeasureText(A_LoopField, "Centre r4 s" . Pause_MainMenu_LabelFontsize . " bold",Pause_MainMenu_LabelFont)
        if (Widthoftext>pauseMainMenuLabelMaxWidth)
            pauseMainMenuLabelMaxWidth := Widthoftext
    }
...
InitializePauseMainMenu: ;Drawing the main menu for the first time (constructing Gui and setting initial parameters)
    ...
    If (PauseMediaObj["Videos"].TotalLabels>0){ ;creating ActiveX video gui
        ...
        try Gui, Pause_GUI31: Add, ActiveX, vwmpVideo, WMPLayer.OCX
            ...
            RLLog.Debug("A Windows Media Player Video exception was thrown: " . e)
        ...
        If (Pause_Loaded <> 1){
            ...
            RLLog.Debug("Windows Media Player Version: " . wmpVersion)
        }
    }   
    ...
    If (SelectedMenuOption="Video"){
        ...
        If(CurrentVideoPlayStatus=3) {
            ...
            RLLog.Debug("VideoPosition at main menu change:" "VideoPosition"videoplayingindex " " VideoPosition%videoplayingindex%)
...
DrawMainMenuBar: ;Drawing Main Menu Bar
    ...
    Loop, parse, Pause_MainMenu_Labels, |
    {
        ...
        If (A_LoopField="Change Disc") { 
            ...
            Gdip_Alt_TextToGraphics(Pause_G25, A_LoopField, Options1, Pause_MainMenu_LabelFont, 0, 0)
            Gdip_Alt_TextToGraphics(Pause_G25, A_LoopField, Options2, Pause_MainMenu_LabelFont, 0, 0)
            Gdip_Alt_TextToGraphics(Pause_G25, A_LoopField, Options3, Pause_MainMenu_LabelFont, 0, 0)            
        }
...
DrawSubMenu: ;Drawing SubMenu Background
    ...
    Alt_UpdateLayeredWindow(Pause_hwnd27, Pause_hdc27,baseScreenWidth-Pause_SubMenu_Width,baseScreenHeight-Pause_SubMenu_Height, Pause_SubMenu_Width, Pause_SubMenu_Height,,monitorTable[pauseMonitor].Left,monitorTable[pauseMonitor].Top,pauseScreenRotationAngle)
    RLLog.Info("Loaded " SelectedMenuOption " SubMenu")
    SubMenuDrawn := 1
Return   
...
SaveState:
    If(VSubMenuItem<>0){
        ...
        if (Pause_SaveStateScreenshot = "true")
            SaveStateBackgroundFile := RIni_GetKeyValue("Stat_sys",dbName,"SaveState" . VSubMenuItem . "Screenshot", false)
...
StateMenuList:
    ...
    If( A_index >= VSubMenuItem){
        ...
        ReadSaveTime := RIni_GetKeyValue("Stat_sys",dbName,"SaveState" . A_index . "SaveTime", "Empty Slot")
    ...
    If(SelectedMenuOption="LoadState"){
        ReadSaveTime := RIni_GetKeyValue("Stat_sys",dbName,"SaveState" . VSubMenuItem . "SaveTime", "Empty Slot")
...
MovesList:
    ...
    If(FullScreenView <> 1){
        ...
        Alt_UpdateLayeredWindow(Pause_hwnd33, Pause_hdc33,baseScreenWidth - HelpTextLenghtWidth - 2*Pause_SubMenu_AdditionalTextMarginContour,baseScreenHeight- Pause_SubMenu_SmallFontSize,HelpTextLenghtWidth+2*Pause_SubMenu_AdditionalTextMarginContour,Pause_SubMenu_HelpFontSize,,monitorTable[pauseMonitor].Left,monitorTable[pauseMonitor].Top,pauseScreenRotationAngle)
        }
    } Else {
...
MoveRight:
    ...
    If (VSubMenuItem=0){
        ...
        Alt_UpdateLayeredWindow(Pause_hwnd25, Pause_hdc25,0,round((baseScreenHeight-Pause_MainMenu_BarHeight)/2)+Pause_MainMenu_BarVerticalOffset, baseScreenWidth, Pause_MainMenu_BarHeight,,monitorTable[pauseMonitor].Left,monitorTable[pauseMonitor].Top,pauseScreenRotationAngle)
        RLLog.Info("Loaded Main Menu Bar. Current Main Menu Label: " Pause_MainMenuSelectedLabel)
        If (SubMenuDrawn=1){
            Gdip_GraphicsClear(Pause_G26)
            Alt_UpdateLayeredWindow(Pause_hwnd26, Pause_hdc26,ConfigMenuX,ConfigMenuY, ConfigMenuWidth, ConfigMenuHeight,,monitorTable[pauseMonitor].Left,monitorTable[pauseMonitor].Top,pauseScreenRotationAngle)
            Gdip_GraphicsClear(Pause_G27)
...
BacktoMenuBar:
    ...
    If (SelectedMenuOption:="Video"){
        ...
        If(CurrentVideoPlayStatus=3) {
            try VideoPosition%videoplayingindex% := wmpVideo.controls.currentPosition
            RLLog.Debug("VideoPosition at back to main menu:" "VideoPosition"videoplayingindex " " VideoPosition%videoplayingindex%)
            try wmpVideo.controls.stop
        }
        Gui,Pause_GUI31: Show, Hide
        Gui, Pause_GUI32: Show
...
ToggleItemSelectStatus:
    ...
    If(SelectedMenuOption="ChangeDisc"){
        ...
        RLLog.Debug("SelectGame - User selected to load: " . selectedRom)
        ...
        If (sevenZEnabled = "true")	; Only need to continue If 7z support is turned on, this check is in case emu supports loading of compressed roms. No need to decompress our rom If it does
            {	
            If Pause_RomExt in %sevenZFormats%	; Check If our selected rom is compressed.
                {	
                RLLog.Debug("SelectGame - This game needs 7z to load. Sending it off for extraction: " . Pause_RomPath . "\" . Pause_DbName . Pause_RomExt)
                ...
                RLLog.Debug("SelectGame - Returned from 7z extraction, path to new rom is: " . selectedRom)
                romTable[HSubMenuItem,19] := Pause_RomPath	; storing path to extracted rom in column 19 so 7zCleanUp knows to delete it later
                RLLog.Debug("SelectGame - Stored """ . Pause_RomPath . """ for deletion in 7zCleanup.")
            } Else {
                RLLog.Debug("SelectGame - This game does not need 7z. Sending it directly to the emu or to Virtual Drive If required.")
            }
            RLLog.Debug("SelectGame - Ended")
        }
        Gosub, ExitPause
    }
...
ExitPause:
    ...
    RLLog.Info("Closing Pause")
    gosub, DisableKeys
    RLLog.Debug("Disabled Keys while exiting")
    Pause_Active:=false
    If not(Pause_MuteSound="true"){ 
        If (Pause_MuteWhenLoading="true"){ ;Mute when exiting Pause to avoiding sound stuttering
            ...
            If !(PauseInitialMuteState){
                setMute(1,emulatorVolumeObject)
                RLLog.Debug("Muting computer sound while Pause is loaded. Master Mute status: " getMute(,emulatorVolumeObject) " (1 is mutted)")
            }
        }
    }    
    if (pauseOnPrimaryMonitor) {
    ...
    If (Pause_KeepPlayingAfterExitingPause="false"){
        If (CurrentMusicPlayStatus=3){
            try wmpMusic.controls.pause
            RLLog.Debug("Pausing music")
        }
    }
    If (SelectedMenuOption="Videos") {
        try VideoPosition%videoplayingindex% := wmpVideo.controls.currentPosition
        RLLog.Debug("VideoPosition at Pause exit:" "VideoPosition"videoplayingindex " " VideoPosition%videoplayingindex%) 
        try wmpVideo.controls.stop
        try wmpVideo.close
    }
    If !disableLoadScreen {
	...
	If !disableSuspendEmu  ;Unsuspending Emulator Process 
        ...
        RLLog.Debug("Emulator process started")
    }
    If !disableRestoreEmu  ;Restoring emulator
        ...
        IfWinNotActive, ahk_class %EmulatorClass%,,%frontendWinTitle% ahk_class %frontendWinClass%
            ...
            RLLog.Debug("Emulator screen reactivated")
        }
    }
    gosub, RestoreEmu
    RLLog.Debug("Loaded emulator specific module restore commands")
    ...
    RLLog.Debug("Setting Pause starting end for subtracting from statistics played time: " Pause_EndTime)
    ...
    RLLog.Debug("Black Screen Gui destroyed")
    If !(rlMode = "pause") {
		StopPauseUserFeatures%zz%()	; stoping pause user functions here so they are triggered right before Pause closes
        ...
        RLLog.Debug("Enabled exit emulator, bezel, and multigame keys")
    ...
    If close_emulator {
        RLLog.Info("Exiting Emulator From Pause")
        gosub, CloseProcess
...
SendCommandstoEmulator:
    If (ItemSelected = 1){
        ...
        If(SelectedMenuOption="ChangeDisc"){
            ...
            RLLog.Debug("PauseExit - Processing MultiGame label in module.")
            gosub, MultiGame%zz%
            RLLog.Debug("PauseExit - Finished Processing MultiGame label.")
        }
    }
Return
...
Pause_ProgressBarAnimation:
	...
	RLLog.Info("Pause_ProgressBarAnimation - Started")
    ...
    Loop {
		...
		Else {	; bar is at 100%
			...
			RLLog.Debug("Pause_ProgressBarAnimation - Bar reached 100%")
			If (fadeBarInfoText = "true")
		...
		{	RLLog.Debug("Pause_ProgressBarAnimation - 7z.exe is no longer running, breaking out of progress loop.")
			Break
		}
	...
	RLLog.Info("Pause_ProgressBarAnimation - Ended")
Return
...
PostProcessingMediaObject(feMedia, ByRef PauseMediaObj){
    Global Pause_MainMenu_Labels, keymapperEnabled, systemName
    ...
    for SubMenuLabel, element2 in PauseMediaObj
        ...
        if !(TotalLabels[SubMenuLabel])
            if !(((SubMenuLabel="Controller") and (keymapperEnabled = "true")) or (SubMenuLabel="HighScore"))
                StringReplace, Pause_MainMenu_Labels, Pause_MainMenu_Labels, %SubMenuLabel%|,
        if mediaAssetsLog
            RLLog.Debug("Media assets found on submenu: " . SubMenuLabel . mediaAssetsLog)
    }
    ...
    If CurrentFileExtension in %ListofSupportedVideos%    
        ...
        If !(FullScreenView){
            If !(AnteriorFilePath = CurrentFilePath) {
                ...
                If(CurrentVideoPlayStatus=3) {
                    try VideoPosition%videoplayingindex% := wmpVideo.controls.currentPosition
                    RLLog.Debug("VideoPosition at video change in videos menu:" "VideoPosition"videoplayingindex " " VideoPosition%videoplayingindex%)
                    try wmpVideo.controls.stop
                    timeout:= A_TickCount
                    Loop
                        {
                ...
                try wmpVideo.controls.pause
                RLLog.Debug("Playing Video File: " CurrentFilePath)
                ...
                RLLog.Debug("Jumping to VideoPosition:" "VideoPosition"VSubMenuItem " " VideoPosition%VSubMenuItem%)
                ...
                If(A_index >= HSubMenuItem){
                    ...
                    If !(AllPagesLoaded%CurrentLabelNumber% = true){
                        IfNotExist, % RLMediaPath . "\" . SubMenuName . "\Temp\" . systemName . "\" . CurrentLabelName . "\" . "page" . A_Index . ".png"
                            ...
                            Alt_UpdateLayeredWindow(Pause_hwnd27, Pause_hdc27,baseScreenWidth-Pause_SubMenu_Width,baseScreenHeight-Pause_SubMenu_Height, Pause_SubMenu_Width, Pause_SubMenu_Height,,monitorTable[pauseMonitor].Left,monitorTable[pauseMonitor].Top,pauseScreenRotationAngle)
                            RLLog.Debug("Loaded PDF page " A_Index " and update " SelectedMenuOption " SubMenu.")
...
ReadMovesListInformation() ;Reading Moves List info
{
    ...
    If (PauseMediaObj["MovesList"].TotalLabels<>0){    ;Loading button images
        ...
        RLLog.Debug("Moves List icons path: " . Pause_MovesListCurrentPath)
...
DisableKeys:
    FunctionRunning := true ;error check function running (necessary to avoid exiting Pause in the middle of function running)
    RLLog.Debug("Disable Pause Keys")
    ...
    RLLog.Debug("Pause Keys Disabled")
    FunctionRunning := false
Return

EnableKeys:
    FunctionRunning := true ;error check function running (necessary to avoid exiting Pause in the middle of function running)
    RLLog.Debug("Enable Pause Keys")
    XHotKeywrapper(navSelectKey,"ToggleItemSelectStatus","ON")
    XHotKeywrapper(navLeftKey,"MoveLeft","ON")
    XHotKeywrapper(navRightKey,"MoveRight","ON")
    XHotKeywrapper(navUpKey,"MoveUp","ON")
    ...
    RLLog.Debug("Pause Keys Enabled")
    FunctionRunning := false
Return
...
;-----------------------------------------------------------------------------------------------------------------------------------------
    ...
    If !FileExist(sevenZPath)
        RLLog.Error("7z.exe not found")
    If !FileExist(Pause_MenuSoundPath . "\menu.wav")
        RLLog.Error("Pause source sound files not found")
    If !FileExist(Pause_IconsImagePath . "\Pause.png")
        RLLog.Error("Pause source image files not found")    
    If !FileExist(Pause_MovesListDataPath . "\*.dat")
        RLLog.Error("No Moves List files available")
    RLLog.Info("RocketLauncher HitoText Path:          " pauseHiToTextPath)
    RLLog.Info("RocketLauncher 7z Path:                " sevenZPath)
    ;Settings hardcoded
    ;Mouse Click Sound
    ...
    RLLog.Debug("Pause variables values: " . PauseVarLog)
Return

RIniPauseLoadVar(gRIniVar,sRIniVar,gsec,gkey,gdefaultvalue="",ssec=0,skey=0,sdefaultvalue="use_global") {
    Global
    If not ssec
        ssec := gsec
    If not skey
    ...
    PauseVarLog .= "`r`n`t`t`t`t`t" . "[" . gsec . "] " . gkey . " = " . X3
	Return X3
}  
...
Pause_MusicPlayer:
    If (Pause_Loaded <> 1){
        try wmpMusic := ComObjCreate("WMPlayer.OCX")
            ...
            RLLog.Debug("A Windows Media Player Music exception was thrown: " . e)
...
UpdateMusicPlayingInfo:
    If (SelectedMenuOption="Sound"){
        ...
        If ((CurrentMusicPositionString<>"") and ((CurrentMusicPlayStatus=2) or (CurrentMusicPlayStatus=3))) {
            Gdip_Alt_TextToGraphics(Pause_G33, CurrentMusicStatusDescription . " - " . CurrentMusicPositionString . " (" . CurrentMusicDurationString . ")", OptionsMusicPlayerText, Pause_SubMenu_Font, 0, 0)
        }
        posHelpY := Pause_SubMenu_Height - Pause_SubMenu_SmallFontSize
        If(VSubMenuItem = 1){
            HelpTextLenghtWidth := MeasureText("Press Left or Right to Change the Volume Level", "Left r4 s" . Pause_SubMenu_HelpFontSize . " Regular",Pause_SubMenu_HelpFont)
            posHelpX := round(Pause_SubMenu_Width - HelpTextLenghtWidth/2 - Pause_SubMenu_AdditionalTextMarginContour)
            OptionsHelp = x%posHelpX% y%posHelpY% Center c%Pause_MainMenu_LabelDisabledColor% r4 s%Pause_SubMenu_HelpFontSize% Regular
            Gdip_Alt_FillRectangle(Pause_G33, Pause_SubMenu_DisabledBrushV, Pause_SubMenu_Width - HelpTextLenghtWidth - 2*Pause_SubMenu_AdditionalTextMarginContour, Pause_SubMenu_Height- Pause_SubMenu_SmallFontSize, HelpTextLenghtWidth+2*Pause_SubMenu_AdditionalTextMarginContour, Pause_SubMenu_HelpFontSize)
            Gdip_Alt_TextToGraphics(Pause_G33, "Press Left or Right to Change the Volume Level", OptionsHelp, Pause_SubMenu_HelpFont, 0, 0)
        }
        If(VSubMenuItem = 2) and (HSubmenuitemSoundVSubmenuitem2=1){
            HelpTextLenghtWidth := MeasureText("Press Select to Change Mute Status", "Left r4 s" . Pause_SubMenu_HelpFontSize . " Regular",Pause_SubMenu_HelpFont)
            posHelpX := round(Pause_SubMenu_Width - HelpTextLenghtWidth/2 - Pause_SubMenu_AdditionalTextMarginContour)
            OptionsHelp = x%posHelpX% y%posHelpY% Center c%Pause_MainMenu_LabelDisabledColor% r4 s%Pause_SubMenu_HelpFontSize% Regular
            Gdip_Alt_FillRectangle(Pause_G33, Pause_SubMenu_DisabledBrushV, Pause_SubMenu_Width - HelpTextLenghtWidth - 2*Pause_SubMenu_AdditionalTextMarginContour, Pause_SubMenu_Height- Pause_SubMenu_SmallFontSize, HelpTextLenghtWidth+2*Pause_SubMenu_AdditionalTextMarginContour, Pause_SubMenu_HelpFontSize)
            Gdip_Alt_TextToGraphics(Pause_G33, "Press Select to Change Mute Status", OptionsHelp, Pause_SubMenu_HelpFont, 0, 0)
        }
        If(VSubMenuItem = 2) and (HSubmenuitemSoundVSubmenuitem2=2){
            HelpTextLenghtWidth := MeasureText("Press Select to Enable Music While Playing the Game", "Left r4 s" . Pause_SubMenu_HelpFontSize . " Regular",Pause_SubMenu_HelpFont)
            posHelpX := round(Pause_SubMenu_Width - HelpTextLenghtWidth/2 - Pause_SubMenu_AdditionalTextMarginContour)
            OptionsHelp = x%posHelpX% y%posHelpY% Center c%Pause_MainMenu_LabelDisabledColor% r4 s%Pause_SubMenu_HelpFontSize% Regular
            Gdip_Alt_FillRectangle(Pause_G33, Pause_SubMenu_DisabledBrushV, Pause_SubMenu_Width - HelpTextLenghtWidth - 2*Pause_SubMenu_AdditionalTextMarginContour, Pause_SubMenu_Height- Pause_SubMenu_SmallFontSize, HelpTextLenghtWidth+2*Pause_SubMenu_AdditionalTextMarginContour, Pause_SubMenu_HelpFontSize)
            Gdip_Alt_TextToGraphics(Pause_G33, "Press Select to Enable Music While Playing the Game", OptionsHelp, Pause_SubMenu_HelpFont, 0, 0)
        }
        If(VSubMenuItem = 3){
            HelpTextLenghtWidth := MeasureText("Press Select to Choose Music Control Option", "Left r4 s" . Pause_SubMenu_HelpFontSize . " Regular",Pause_SubMenu_HelpFont)
            posHelpX := round(Pause_SubMenu_Width - HelpTextLenghtWidth/2 - Pause_SubMenu_AdditionalTextMarginContour)
            OptionsHelp = x%posHelpX% y%posHelpY% Center c%Pause_MainMenu_LabelDisabledColor% r4 s%Pause_SubMenu_HelpFontSize% Regular
            Gdip_Alt_FillRectangle(Pause_G33, Pause_SubMenu_DisabledBrushV, Pause_SubMenu_Width - HelpTextLenghtWidth - 2*Pause_SubMenu_AdditionalTextMarginContour, Pause_SubMenu_Height- Pause_SubMenu_SmallFontSize, HelpTextLenghtWidth+2*Pause_SubMenu_AdditionalTextMarginContour, Pause_SubMenu_HelpFontSize)
            Gdip_Alt_TextToGraphics(Pause_G33, "Press Select to Choose Music Control Option", OptionsHelp, Pause_SubMenu_HelpFont, 0, 0)
        }
        Alt_UpdateLayeredWindow(Pause_hwnd33, Pause_hdc33, baseScreenWidth - Pause_SubMenu_Width,baseScreenHeight-Pause_SubMenu_Height,Pause_SubMenu_Width,Pause_SubMenu_Height,,monitorTable[pauseMonitor].Left,monitorTable[pauseMonitor].Top,pauseScreenRotationAngle)
    } Else {
        Gdip_GraphicsClear(Pause_G33)     
        Alt_UpdateLayeredWindow(Pause_hwnd33, Pause_hdc33, baseScreenWidth - Pause_SubMenu_Width,baseScreenHeight-Pause_SubMenu_Height,Pause_SubMenu_Width,Pause_SubMenu_Height,,monitorTable[pauseMonitor].Left,monitorTable[pauseMonitor].Top,pauseScreenRotationAngle)            
    }
...
UpdateVideoPlayingInfo:
    If (SelectedMenuOption="Videos") and (VSubMenuItem <> 0){
        ...
        If ((CurrentVideoPositionString<>"") and ((CurrentVideoPlayStatus=2) or (CurrentVideoPlayStatus=3)))
            Gdip_Alt_TextToGraphics(Pause_G33, CurrentVideoStatusDescription . " - " . CurrentVideoPositionString . " (" . CurrentVideoDurationString . ")", OptionsVideoPlayerText, Pause_SubMenu_Font, 0, 0)
        posHelpY := Pause_SubMenu_Height - Pause_SubMenu_SmallFontSize
        If(HSubMenuItem = 1){
            HelpTextLenghtWidth := MeasureText("Press Up or Down to Select the Video and Left or Right to Control the Video Playing", "Left r4 s" . Pause_SubMenu_HelpFontSize . " Regular",Pause_SubMenu_HelpFont)
            posHelpX := round(Pause_SubMenu_Width - HelpTextLenghtWidth/2 - Pause_SubMenu_AdditionalTextMarginContour)
            OptionsHelp = x%posHelpX% y%posHelpY% Center c%Pause_MainMenu_LabelDisabledColor% r4 s%Pause_SubMenu_HelpFontSize% Regular
            Gdip_Alt_FillRectangle(Pause_G33, Pause_SubMenu_DisabledBrushV, Pause_SubMenu_Width - HelpTextLenghtWidth - 2*Pause_SubMenu_AdditionalTextMarginContour, Pause_SubMenu_Height- Pause_SubMenu_SmallFontSize, HelpTextLenghtWidth+2*Pause_SubMenu_AdditionalTextMarginContour, Pause_SubMenu_HelpFontSize)
            Gdip_Alt_TextToGraphics(Pause_G33, "Press Up or Down to Select the Video and Left or Right to Control the Video Playing", OptionsHelp, Pause_SubMenu_HelpFont, 0, 0)
        ...
        If(Pause_ArtworkMenuEnabled="true"){
            ...
            If Pause_Running
                If(SelectedMenuOption="Artwork")
                    gosub, DrawSubMenu
            If Pause_MainMenu_Labels not contains Artwork
                {
                Pause_MainMenu_Labels .= "|Artwork" 
                TotalMainMenuItems++
                Gdip_GraphicsClear(Pause_G25)
                Gosub DrawMainMenuBar
                Alt_UpdateLayeredWindow(Pause_hwnd25, Pause_hdc25,0,round((baseScreenHeight-Pause_MainMenu_BarHeight)/2)+Pause_MainMenu_BarVerticalOffset, baseScreenWidth, Pause_MainMenu_BarHeight,,monitorTable[pauseMonitor].Left,monitorTable[pauseMonitor].Top,pauseScreenRotationAngle)
...
pauseMouseClick:
    ...
    If(Pause_MouseClickSound = "true") {
        ...
        RLLog.Debug("Selected Mouse Click Sound: " . MouseRndmSoundPath)
    }
...
wmpVideo_Error(wmpVideo) {
    ...
    RLLog.Debug("A Windows Media Player Video exception was thrown: " . ErrorDescription)
Return
} 
...
wmpMusic_Error(wmpMusic) {
    ...
    RLLog.Debug("A Windows Media Player Music exception was thrown: " . ErrorDescription)
Return
} 
