...
XHotkeyTableCreation(KeyGroup,LabelForSingleKey="",Options="",LabelForHoldKey="",KeyHoldWait="",LabelForDoubleClick="",LabelForMoreThanTwoClicks="",DoubleClickKeyWait=""){
	Global
	Log("XHotkeyTableCreation - Started",5)
	...
	If KeyGroup not in %TableKeyGroups% 
		{
		Log("XHotkeyTableCreation - KeyGroup " . KeyGroup . " not in TableKeyGroups " . TableKeyGroups,5)
		KeyGroupNumber++
		...
		Log("XHotkeyTableCreation - Post loop 1 check",5)
		...
		Log("XHotkeyTableCreation - Post loop 2 check",5)
		...
		Log("XHotkeyTableCreation - Ended",5)
		Return XHotKeyTable
	} Else {
		Log("XHotkeyTableCreation - KeyGroup " . KeyGroup . " is in TableKeyGroups " . TableKeyGroups,5)
		currentGroup:=0
		Loop, %KeyGroupNumber%
			{	
			currentGroup++
		...
		Log("XHotkeyTableCreation - Post loop 10 check",5)
			If not InStr(options, "OFF"){
				If	LabelForSingleKey
					XHotKeyTable[currentGroup,1,13] := true
				If	LabelForHoldKey
			...
			Log("XHotkeyTableCreation - Post loop 11 check",5)
			If (((Instr(KeyGroup,"joy")) and (XHotKeyTable[currentGroup,1,3]=1)) or ( not(Instr(KeyGroup,"joy")) and (XHotKeyTable[currentGroup,1,3]<=2))) and (((LabelForHoldKey="") and (LabelForDoubleClick="") and (LabelForMoreThanTwoClicks="") )){ ; defining normal hotkey command If the key group has: only one joy button or If it is a single key press with one or two simultaneous keys 
				Log("XHotkeyTableCreation - " . (If options = "OFF" ? "Disabling """ . LabelForSingleKey . """ standard Hotkey method for key(s): """ . KeyGroupWithSpaces . """" : "Using standard Hotkey method for key(s): """ . KeyGroupWithSpaces . """ to call label: """ . LabelForSingleKey . """"),5)
				Hotkey, % KeyGroupWithSpaces, %LabelForSingleKey%, %Options% ; normal hotkey command
			} Else { ; defining extended hotkey command If there are more them two simultaneous keys pressed (more them one for joy), or If it has hold, double or more click presses
			...
			Log("XHotkeyTableCreation - Post loop 12 check",5)
		Log("XHotkeyTableCreation - Ended",5)
		Return
	}
}
...
XHotkeyAllKeysPressed(keysToBeChecked){
	Log("XHotkeyAllKeysPressed - Started, checking for these keys to be pressed: """ . keysToBeChecked . """",5)
	Loop, parse, keysToBeChecked, |, %A_Space%
		...
		If ExitKeysPressed {
			Log("XHotkeyAllKeysPressed - Ended: Keys being checked were pressed",5)
			Return 1
		}
	}
	Log("XHotkeyAllKeysPressed - Ended: Keys being checked were not pressed",5)
	Return 0
}
