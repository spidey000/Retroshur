...
;
; You do not need to set an emulator for this module because RocketLauncher will look in the "Module Extensions\dxwnd" folder folder for dxwnd.
; Because of this, the module will need to be set as a Virtual Emulator in RocketLauncherUI and you must set skipchecks to "Rom and Emu".
; If you prefer to use your own DxWnd installation you can still do it, simply setup the emulator as usual in RLUI and point it to your DxWnd installation folder. You should disable skipchecks in this case.
;
...
; You may have to set Skipchecks to "Rom Only" or "Rom and Emu", otherwise RocketLauncher will error looking for a rom if your exe/bat/lnk is not the same name as you have in your xml.
;
...
If (emuPath) { ;User wants to use his own Dxwnd install folder
	dxwndFullPath := emuPath . "\" . executable
	DxwndUpdateIniPath()
}
...
hideEmuObj := Object(titleClass,1)	; Hide_Emu will hide these windows. 0 = will never unhide, 1 = will unhide later
HideEmuStart()
...
MaximizeDxWndWindow(titleClass)
HideEmuEnd()
FadeInExit()
...
;	Sleep, 500 ;Uncomment these 2 lines if your FE doesn't regain focus after closing a vertical game, sloppy but it works until something else is thought of
