...
;----------------------------------------------------------------------------
; Notes:
; To use the built-in roms for this emu, set SkipChecks to "Rom Only" in RocketLauncherUI.
...
; In order to use the built-in overlays, the romName is being converted to the emu's built in game id found in the configuration.xml. 
; This avoids having to edit the xml manually to change it to No-Intro naming standards. We also don't have to setup overlay files this way too!
;----------------------------------------------------------------------------
StartModule()
BezelGUI()
FadeInStart()
...
If (!gameID) { ;External ROM Loading (This won't allow support for artwork such as overlays)
...
} Else { ;Internal ROM Loading
	;Read configuration.xml, get rompath and if rom doesn't exist copy it from romPath
	If FileExist(romPath . "\" . romName . romExtension) {
		Fileread, ConfxmlContents, %emuPath%\data\configuration.xml
		RegExMatch(ConfxmlContents, "s)<game.*id=" . """" . gameID . """" . ".* cartridge="  . """" . ".*""", internalrompath)
		RegExMatch(internalrompath,"cartridge=" . """" . ".*""", internalrompath)
		StringTrimLeft, internalrompath, internalrompath, 11
		StringTrimRight, internalrompath, internalrompath, 1

		If (internalrompath) {
			internalrompath := emuPath . "\" . internalrompath
			StringReplace, internalrompath, internalrompath, /, \, A
			If !FileExist(internalrompath) { ;Copy from romPath to internal rom path
				Log("Copied '" . romPath . "\" . romName . romExtension . "' to '" . internalrompath . "'")
				FileCopy, %romPath%\%romName%%romExtension%, %internalrompath%
			}
		}
	}
}
