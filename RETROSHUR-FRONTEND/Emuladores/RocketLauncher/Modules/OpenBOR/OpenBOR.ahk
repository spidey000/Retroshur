...
; If you don't want to use the above path/exe, change the rom's module settings in RocketLauncherUI and define a path for each game.
...
; Escape will only close the game from the main menu, it is needed for in-game menu usage otherwise. Using Escape as your RocketLauncher exit key is not advised as it is needed for in-game usage.
...
backupDirectory := romPath . "\Saves\" . romName
backupCFGfiles := romPath . "\Saves\" . romName . "\" . "*.*"
...
7z(romPath, romName, romExtension, sevenZExtractPath,,,1)		; allowing larger sized folders on sevenZExtractPath
...
If FileExist(romPath . "\" . gamePath) && (gamePath && gamePath != "ERROR") {
	...
	ScriptError("Could not find " . gamePath . "`nPlease place your game in its own folder in your Rom_Path or define a custom gamePath in RocketLauncherUI's module setting for OpenBOR")
...
; Overwrite configuration files to 7zipped location if they exist
extractedCFGfiles := gPath . "\Saves\*.*"
If sevenZEnabled
	If FileExist(backupCFGfiles)
		FileCopy, %backupCFGfiles%, %extractedCFGfiles%, 1
...
errlvl := Run(gExe, gPath, "UseErrorLevel", game_PID)
If errlvl
...
If sevenZEnabled {
	If !FileExist(backupDirectory)
		FileCreateDir, %backupDirectory%
	FileCopy, %extractedCFGfiles%, %backupCFGfiles%, 1
}
...
HaltEmu:
	If altExitKey
		XHotKeywrapper(altExitKey,"CloseProcess","OFF")
Return
