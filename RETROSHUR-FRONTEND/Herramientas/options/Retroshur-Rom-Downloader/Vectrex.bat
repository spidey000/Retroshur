...
@echo off & Cls & Color 0E & Title Retroshur Rom Downloader: GCE Vetrex (26.3 MB)
SET HOME="%~dp0"
...
set "LISTA=https://mega.nz/#!mxUWxYTD!VfEX-gvnjDeRWIIK1O5cdwR3Y5QLPxgyURqRzqeIkeU"
echo(
echo Creando carpetas temporales
if not exist %TEMPORALES% mkdir %TEMPORALES%
timeout 2 > NUL
echo Hecho!
echo Descargando Backup GCE Vectrex para Retroshur
timeout 2 > NUL
%BAJAR% --print-names %LISTA% --path %TEMPORALES%
cd %TEMPORALES%
%EXTRAER% x vectrex.zip -o%CARPETA% -aoa 
cd %TEMPORALES%\..\..
